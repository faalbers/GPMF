...
GPMF::ACCL::ACCL(std::string filePath, uint64_t filePos, std::string pathParent)
    : klv(filePath, filePos, pathParent)
{
    // throw an error if at one point they decide to change the data type
    if ( dataType != 's' )
        throw std::runtime_error("ACCL klv wrong data type: "+std::string(1, dataType));
    if ( sampleSize != 6 )
        throw std::runtime_error("ACCL klv wrong sample size: " + std::to_string(sampleSize));

    std::ifstream fileStream(filePath, std::ios::binary);
    if ( fileStream.fail() ) throw std::runtime_error("ACCL klv can not parse file: "+filePath);
...
void GPMF::ACCL::printData(bool fullLists)
{
    ...
    std::cout << path_ << " ('" << dataType << "' " << sampleSize << " " << dataRepeat << ") : accelerometer data" << std::endl;
    int index = 1;
