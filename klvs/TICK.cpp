    ...
    : klv(dataString, pathParent)
{
    ...
    if ( dataType != 'L' )
        error_("TICK klv wrong data type: "+std::string((char *)&dataType).substr(0,1));
    if ( dataRepeat != 1 )
        error_("TICK klv has more then value: " + std::to_string(dataRepeat));
...
void GPMF::TICK::printHierarchyData(bool fullLists)
{
    printData(fullLists);
    for ( auto child : children_ ) child->printHierarchyData(fullLists);
}

std::string GPMF::TICK::key = "TICK";
